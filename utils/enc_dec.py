...
def main() -> None:
    ...
    print()
    print("====== Test encode -----------------------------------------")
    data = bytes(bytearray.fromhex("01640ce40c3eb31500003e0a1a0000"))  # BTHome data (not encrypted)
    parse_value(data)  # Print temperature and humidity

    print()
    print("Preparing data for encryption")
    count_id = bytes(bytearray.fromhex("01000000"))  # count id (change every message)
    mac = binascii.unhexlify("a4c138c3bd16")  # MAC
    uuid16 = b"\xD2\xFC"
    sw_version = b"\x41"
    bindkey = binascii.unhexlify("000102030405060708090a0b0c0d0e0f")
